...
def refresh_token_sync(uid, password):
    ...
    try:
        ...
        url = f"http://shappno-jwt-api.vercel.app/token?uid={uid}&password={encoded_password}"
...
def load_accounts():
    ...
    print(f"✅ Loaded {len(accounts)} accounts from rg_rifat_07.txt")
    return accounts

# ============ SAVE TOKENS TO FILE ============
def save_tokens_to_file(tokens):
    """Save tokens back to rg_rifat_07.txt"""
    try:
        # Read existing lines
        lines = []
        with open("rg_rifat_07.txt", 'r') as f:
            lines = f.readlines()
        ...
        # Write back
        with open("rg_rifat_07.txt", 'w') as f:
            f.writelines(new_lines)
        
        print(f"✅ Updated rg_rifat_07.txt with {len(tokens)} tokens")
        return True
    except Exception as e:
...
def home():
    return jsonify({
        "name": "RG RIFAT 07 API",
        "version": "2.0",
        "features": [
            "No API key required",
            "Auto token refresh on each API call",
            "Reads from rg_rifat_07.txt"
        ],
        "endpoints": {
...
if __name__ == '__main__':
    print("🚀 RG RIFAT 07 API Started!")
    print("✅ No API key required")
    print("✅ Auto refresh: Active (on each API call)")
    print("📁 Reading from: rg_rifat_07.txt")
